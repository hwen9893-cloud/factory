...
class OpenAIProvider(Provider):
    """OpenAI Python SDK. Also used for OpenRouter and any OpenAI-compatible base_url."""
...
def _read_key(env_name: str, provider: str) -> str:
    ...
    fallback = DEFAULT_KEY_ENV.get(provider, "")
...
def build_provider(profile: ModelProfile) -> Provider:
    ...
    if not key:
        hint = profile.api_key_env or DEFAULT_KEY_ENV.get(name) or "the provider API key env var"
        raise ProviderError(f"missing API key for {name}. Set {hint}.")
    if name in {"openai", "openai_compat", "openrouter"}:
        base = (profile.base_url or DEFAULT_BASE_URL.get(name) or DEFAULT_BASE_URL["openai"]).rstrip("/")
